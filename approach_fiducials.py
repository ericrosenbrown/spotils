...
def main(argv):
    ...
    parser.add_argument('--fiducial', type=int, help='Fiducial to approach')
    ...
    #### seed_tform_object gives me pose of the object in the map frame. Great!
    seed_tform_fiducial = current_anchored_world_objects['523'][0].seed_tform_object
    
    #Turn protobuf SE3Pose into math SE3Pose
    se3pose_fiducial = SE3Pose(x=seed_tform_fiducial.position.x,
                            y=seed_tform_fiducial.position.y,
                            z=seed_tform_fiducial.position.z,
                            rot=seed_tform_fiducial.rotation)

    #Construct SE3 pose relative to fiducial we want for approach
    se3pose_approach = SE3Pose(x=0,
                            y=0,
                            z=1.5,
                            rot=Quat(w=1,x=0,y=0,z=0))

    
    other = se3pose_fiducial.mult(se3pose_approach)
    print(other)
    print(other.rotation.to_yaw())
